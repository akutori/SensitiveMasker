...
/// (scrypt)は約150バイト、鍵ファイル用(x25519)は、ageが、長さの決まらないダミーの受け手(grease)を足すため、約180〜360バイト
/// (毎回変わる)である。ageのヘッダーの解析は、ヘッダーの長さの二乗の時間を要する(1バイトずつ読み足しては、最初から解析し直す)
/// ため、細工された巨大なヘッダーで、長時間ハングしないよう、上限を超えるヘッダーは、解析せずに拒否する。上限は、正規のヘッダーの
/// 最大(約360バイト)の5倍以上で、上限ちょうどのヘッダーの解析は、リリースビルドで数ミリ秒である。
