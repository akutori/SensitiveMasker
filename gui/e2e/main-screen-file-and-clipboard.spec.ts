...
// OSネイティブのファイルダイアログ(plugin:dialog|open・plugin:dialog|save)を呼ぶため、
// export-import.spec.tsと同じ理由(ネイティブダイアログはWebDriver自動化の対象外で、
// 応答を待ち続けてハングする)でこのファイルでは検証しない。実機での手動確認が必要。
//
...
// 「再読み込みボタン自体が無ければ反映されない」ことの検証にならない(実際に最初の
// 実装ではこれに気付かず誤ったテストになっていた)。そのため、実際に別プロセスとして
